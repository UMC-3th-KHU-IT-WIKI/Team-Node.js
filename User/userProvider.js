...
import { selectUser, selectUserAccount, selectUserEmail, selectUserId, selectUserInfo, selectUserPassword } from "./userDao";
...
export const retrieveUserInfoList = async(page)=>{
    const connection = await pool.getConnection(async conn => conn);
    const result = await selectUserInfo(connection,page);
    return result
}